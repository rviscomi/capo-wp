...
class Site_Health {
	...
	public function register_tests( $tests ) {
		$tests['direct']['capo_head_optimization'] = array(
			'label' => __( 'HTML <head> Optimization (Capo)', 'capo-head-optimizer' ),
			'test'  => array( $this, 'test_capo_status' ),
		);

		$tests['direct']['capo_head_hygiene'] = array(
			'label' => __( 'HTML Head Hygiene & Validation (Capo)', 'capo-head-optimizer' ),
	...
	public function test_capo_status() {
		...
		if ( ! $is_enabled ) {
			return array(
				'label'       => __( 'Capo head optimization is disabled', 'capo-head-optimizer' ),
				'status'      => 'recommended',
				'badge'       => array(
					'label' => __( 'Performance', 'capo-head-optimizer' ),
					'color' => 'orange',
				),
				'description' => sprintf(
					'<p>%s</p>',
					__( 'Capo is currently disabled in your settings. Enabling Capo reorders critical head elements to improve First Contentful Paint (FCP) and Largest Contentful Paint (LCP).', 'capo-head-optimizer' )
				),
				'actions'     => sprintf(
					'<p><a class="button button-primary" href="%s">%s</a></p>',
					esc_url( admin_url( 'options-general.php?page=capo' ) ),
					__( 'Enable Capo in Settings', 'capo-head-optimizer' )
				),
		...
		return array(
			'label'       => __( 'HTML <head> is optimized by Capo', 'capo-head-optimizer' ),
			'status'      => 'good',
			'badge'       => array(
				'label' => __( 'Performance', 'capo-head-optimizer' ),
				'color' => 'blue',
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'Capo is actively optimizing the order of your HTML &lt;head&gt; tags to ensure fast resource discovery, early network connections, and unblocked rendering.', 'capo-head-optimizer' )
			),
			'actions'     => '',
			'test'        => 'capo_head_optimization',
		);
	...
	public function test_capo_hygiene() {
		...
		if ( is_wp_error( $response ) ) {
			return array(
				'label'       => __( 'Could not verify <head> hygiene via loopback', 'capo-head-optimizer' ),
				'status'      => 'good',
				'badge'       => array(
					'label' => __( 'Performance', 'capo-head-optimizer' ),
					'color' => 'blue',
				...
				'description' => sprintf(
					'<p>%s</p>',
					__( 'Loopback request to the homepage timed out or failed. You can test your site directly using the live demo link in Capo Settings.', 'capo-head-optimizer' )
				),
				'actions'     => '',
		...
		if ( empty( $html ) || ! preg_match( '/<head(\s[^>]*)?>(.*?)<\/head>/is', $html, $matches ) ) {
			return array(
				...
				'description' => '<p>' . __( 'The homepage did not return a valid HTML &lt;head&gt; section.', 'capo-head-optimizer' ) . '</p>',
				'actions'     => '',
				'test'        => 'capo_head_hygiene',
			);
		}
		...
		if ( ! empty( $warnings ) ) {
			...
			return array(
				'label'       => sprintf(
					...
					_n( 'Capo detected %d <head> hygiene issue on your homepage', 'Capo detected %d <head> hygiene issues on your homepage', count( $warnings ), 'capo-head-optimizer' ),
					count( $warnings )
				),
				'status'      => 'recommended',
				'badge'       => array(
				...
				'description' => sprintf(
					...
					__( 'The following issues were detected in your &lt;head&gt; markup, which may impact browser rendering speed, SEO, or layout stability:', 'capo-head-optimizer' ),
					$warning_items,
					__( 'Check your active theme templates, custom code snippets, or header injection settings to remove or fix these offending elements.', 'capo-head-optimizer' )
				),
				'actions'     => sprintf(
					'<p><a class="button button-primary" href="%s">%s</a></p>',
					esc_url( admin_url( 'options-general.php?page=capo' ) ),
					__( 'Capo Settings', 'capo-head-optimizer' )
				),
				'test'        => 'capo_head_hygiene',
			);
		}
		...
		return array(
			'label'       => __( 'No HTML <head> hygiene issues detected', 'capo-head-optimizer' ),
			'status'      => 'good',
			'badge'       => array(
				'label' => __( 'Performance', 'capo-head-optimizer' ),
				'color' => 'blue',
			...
			'description' => sprintf(
				'<p>%s</p>',
				__( 'Your &lt;head&gt; markup is clean! All critical tags (single &lt;title&gt;, valid &lt;meta viewport&gt;, and resource preloads) adhere to web performance best practices.', 'capo-head-optimizer' )
			),
			'actions'     => '',
			'test'        => 'capo_head_hygiene',
		);
